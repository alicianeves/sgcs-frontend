...
export default function PessoasPage({
  ...
  return (
    <div className="min-h-dvh bg-[#F7F8FA] text-[#273440] lg:flex">
      {menuAberto && (
        <button
          type="button"
          aria-label="Fechar menu"
          className="fixed inset-0 z-30 bg-[#1c3040]/45 lg:hidden"
          onClick={() => setMenuAberto(false)}
        />
      )}
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col bg-[#0d5d86] px-3 py-5 text-white transition-transform lg:sticky lg:top-0 lg:h-dvh lg:translate-x-0 ${menuAberto ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center gap-3 px-2">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold leading-tight">Centro Social</p>
            <p className="text-xs text-white/75">Santa Rita de Cássia</p>
          </div>
          <button type="button" className="ml-auto lg:hidden" onClick={() => setMenuAberto(false)} aria-label="Fechar menu">
            <X size={20} />
          </button>
        </div>

        <div className="mt-12 px-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/55">
          Módulos
        </div>
        <nav aria-label="Menu principal" className="mt-3 space-y-1">
          <div className="flex items-center gap-3 rounded-lg bg-white/15 px-3 py-2.5 text-sm font-medium">
            <UserRoundCog size={18} aria-hidden="true" />
            Administração
          </div>
          <button
            type="button"
            onClick={() => { voltar(); setMenuAberto(false); }}
            className="ml-4 flex w-[calc(100%-1rem)] items-center gap-3 rounded-lg bg-white/10 px-3 py-2.5 text-left text-sm transition-colors hover:bg-white/20"
          >
        ...
        <div className="mt-auto border-t border-white/15 pt-4">
          <button type="button" onClick={onSair} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-white/85 hover:bg-white/10">
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between gap-4 border-b border-[#d9e1ea] bg-white px-5 sm:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <button type="button" className="lg:hidden" onClick={() => setMenuAberto(true)} aria-label="Abrir menu">
              <Menu size={22} />
            </button>
            <span className="truncate text-sm text-[#606b79]">Sistema de Gestão · Centro Social Santa Rita de Cássia</span>
          </div>
          <div className="flex shrink-0 items-center gap-2.5">
            <span className="flex size-9 items-center justify-center rounded-full bg-[#1495D6] text-xs font-semibold text-white">{iniciais}</span>
            <div className="hidden text-sm leading-tight sm:block">
              <p className="max-w-36 truncate font-medium">{usuarioAtual}</p>
              <p className="text-xs text-[#606b79]">Administração</p>
            </div>
          </div>
        </header>
          ...
          {!formularioAberto ? (
            <>
              ...
              <div className="mb-6 grid gap-4 sm:grid-cols-3">
                {[
                  { titulo: "Total de pessoas", total: pessoas.length, Icone: CircleUserRound },
                  { titulo: "Ativas", total: pessoas.filter((p) => p.status).length, Icone: Check },
                  { titulo: "Inativas", total: pessoas.filter((p) => !p.status).length, Icone: UsersRound },
                ].map(({ titulo, total, Icone }) => (
                  <div key={titulo} className="flex items-center justify-between rounded-2xl border border-[#d9e1ea] bg-white p-5 shadow-sm">
                    <div><p className="text-sm text-[#606b79]">{titulo}</p><p className="mt-2 text-2xl font-bold">{total}</p></div>
                    <span className="flex size-10 items-center justify-center rounded-xl bg-[#eaf7ff] text-[#1495D6]"><Icone size={20} /></span>
                  </div>
                ))}
              </div>
